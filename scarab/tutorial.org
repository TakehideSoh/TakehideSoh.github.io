...
* 実行の方法
  ...
  - Scala と同じく3つの方法でプログラムを記述し，実行できる．
    ...
    - ブラウザで実行する
      - [[https://mybinder.org/v2/gh/TakehideSoh/Scarab4Binder/master?urlpath=lab/tree/index.ipynb][ここ]] をクリックしてもらうだけで，少し待つと実行環境が表示されます．
      - 環境設定無しで, ブラウザだけで実行できます.
      - この仕組みは以下2つのリポジトリのフォークである [[https://github.com/TakehideSoh/Scarab4Binder][Scarab4Binder]] 
        で提供されます. 
