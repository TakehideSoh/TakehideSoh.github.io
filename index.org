...
*** (*PI*) {{{item(Research and Development of a New SAT-based Parallel CSP Solver using Multiple Constraint Modelings and SAT Encodings)}}}
...
*** {{{others(JSPS Bilateral Programs (Joint Research Projects) Joint Research Projects with FRANCE (MEAE-MESRI) ``SAKURA Program'')}}} 
...
*** (COI) {{{item(背景理論付き解集合プログラミングに関する研究開発)}}} {{{going}}}                                             
