...
sealed abstract class Term extends Expr {
  ...
  /** Returns the substraction of `b` from the term. */
  def - (b: Int): Sum = toSum.sub(b)
  /** Returns the substraction of `x` from the term. */
...
object Var {
  ...
  def apply() = {
    count += 1; val x = new Var("TMP_I_" + count); x.aux = true
    x
  }
}
...
sealed abstract class Constraint extends Expr {
  ...
  def unary_! : Constraint = this match {
    case p: Bool => Not(p)
    case Not(p) => p
    case LeZero(sum) => LeZero(sum.neg.add(1))
...
object Bool {
  ...
  /** Returns a new anonymous variable. */
  def apply() = {
    count += 1; val p = new Bool("TMP_B_" + count); p.aux = true
    p
  }
}
 ...
 */
case class Domain(lb: Int, ub: Int) {
  require(lb <= ub)
}
               ...
               var constraints: IndexedSeq[Constraint] = IndexedSeq.empty)
{
  ...
  def add(c: Constraint): Constraint = {
    constraints = constraints :+ c; changed = true; c
  }
